...
-- select 쿼리문을 저장한 객체로 가상테이블이라고 블린다.
...
-- 필드 목록이 없으면 join view 'gangnam.hansik2'에 삽입할 수 없습니다.
...
-- join으로 생성된 view의 데이터를 삭제하는 일도 권장하지 않음
...
-- view 생성시 지시한 컬럼의 별칭이 view의 컬럼명이 됨
select
	*
  from hansik2
 where 메뉴명 like '%수정%';
